const passwdServ = require('./passwd');
...
describe('Password service:', () => {
  ...
  describe('bcrypt:', () => {
    it('should be able to hash & compare passwords using bcrypt', async () => {
      ...
      const hash1 = await passwdServ.bcrypt.hash(plainPasswd);
      const hash2 = await passwdServ.bcrypt.hash(plainPasswd);
      expect(hash1 !== hash2).toBe(true);
      expect(await passwdServ.bcrypt.compare(plainPasswd, hash1)).toBe(true);
      expect(await passwdServ.bcrypt.compare(plainPasswd, hash2)).toBe(true);
    });
  });
  ...
  describe('Password helpers:', () => {
    it('should add salt to the end of passwword', async () => {
      const salt = 'abcd';
      const { passwd } = await passwdServ.getSaltedPasswd(plainPasswd, salt);
    ...
    it('should create salt if not provided', async () => {
      const { passwd, salt } = await passwdServ.getSaltedPasswd(plainPasswd);
      expect(passwd.indexOf(plainPasswd)).toBe(0);
      expect(passwd.length - salt.length).toBe(plainPasswd.length);
    });
    ...
    it('should throw error if "password" argument is invalid', async () => {
      try {
        await passwdServ.getSaltedPasswd();
      } catch (err) {
