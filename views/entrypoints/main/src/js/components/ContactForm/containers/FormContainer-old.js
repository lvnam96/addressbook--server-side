...
import { getRandomStr, getRandomHexColor } from '../../../helpers/utilsHelper';
...
import { findActivatingCbook } from '../../../helpers/findHelper';
...
class FormContainer extends React.Component {
  ...
  handlerSaveForm (e) {
    ...
    submittedData.website = ((ws) => {
      ...
      if (ws.length) {
        const hasURLSyntax = ws.search(/^https?:\/\/\S+/g) === 0;
        const hasOnlyProtocol = ws.search(/^https?:\/\/$|^h?ttps?:\/\/$|^ht?tps?:\/\/$|^http?s?:\/\/$/g) === 0;
        if (hasURLSyntax) {
    ...
    // format labels data
    let newLabels = [];
    if (this.cboxFamily.current.checked) {
      newLabels.push('family');
    }
    if (this.cboxCoWorker.current.checked) {
      newLabels.push('coWorker');
    }
    if (this.cboxFriends.current.checked) {
      newLabels.push('friends');
    }
    ...
    const currentCbookId = findActivatingCbook(adbk.redux.store.getState().cbooks);
    submittedData.adrsbookId = currentCbookId;
    submittedData.accountId = adbk.inst.user.id;
    const contact = new adbk.classes.Contact(submittedData);
    // OR:
    // const contact = adbk.contactCtrler.newContact(submittedData);// controller is not finished
