...
let needToBeReSorted = false;
let contactsList = JSON.parse(fs.readFileSync('./js/data.json', 'UTF-8')) || [];

const time = timeObj;
const isStorageAvailable = checkStorageAvailable('localStorage');
const ____shouldBeSaved = () => {
  return isModified;
};
const ____dontSaveDataToLocalStorageAgain = () => {
  isModified = false;
};
const ____saveDataToLocalStorage = () => {
  localStorage.contactsList = JSON.stringify(contactsList);
};
const ____rangeOfWeek = (today = time.curDay, toDate = time.curDate) => {
  // Calculate range (array) of days in current week
  const result = [];
  // toDate = testDateNum > -1 ? testDateNum : time.curDate,
  const thisMonth = time.curMonth;
  const thisYear = time.curYear;
  ...
  const dayInNextMonth = thisMonth === 12 ? daysInMonth(1, thisYear + 1) : daysInMonth(thisMonth + 1, thisYear);
...
const ____getBirthsInMonth = (month = time.curMonth) => {
  return contactsList
    .filter((contact) => parseInt(contact.birth.split('-')[1], 10) === month)
    .sort((a, b) => {
      let birthA = parseInt(a.birth.split('-')[2], 10);
      let birthB = parseInt(b.birth.split('-')[2], 10);
      return birthA - birthB;
    });
};
const ____getBirthsInWeek = (dayInWeekArr = rangeOfWeek()) => {
  const curDay = time.curDay;
  const curMonth = time.curMonth;
  const curYear = time.curYear;
  const birthsInLastMonth = getBirthsInMonth(curMonth - 1 === 0 ? 12 : curMonth - 1);
  const birthsInCurrentMonth = getBirthsInMonth(curMonth);
  const birthsInNextMonth = getBirthsInMonth(curMonth + 1 === 13 ? 1 : curMonth + 1);
    ...
    if (curDay > 15) {
      ...
      arr1 = birthsInCurrentMonth
        ...
        .sort((a, b) => {
          ...
          const birthB = parseInt(b.birth.split('-')[2], 10);
          return birthA - birthB;
        });

      arr2 = birthsInNextMonth
        .filter((contact) => {
          const birth = parseInt(contact.birth.split('-')[2], 10);
...
const ____filterBirthsToday = () => {
  ...
  contactsList.forEach((contact) => {
    contact.hpbd =
      birthsToday.findIndex((contactHasBirthToday) => {
...
const dataNeedToBeSorted = () => {
  needToBeReSorted = true;
};
const dontSortAgain = () => {
  needToBeReSorted = false;
};
const ____sortContactsList = () => {
  // sort by name
...
const ____listLength = () => {
  return contactsList.length;
};
const ____addContact = (newPersonObj) => {
  contactsList = [...contactsList, newPersonObj];
  isModified = true;
  needToBeReSorted = true;
};
const ____replaceData = (newData = contactsList) => {
  contactsList = newData;
};
const ____find = (IDList, callbackFunc, callbackObj) => {
  ...
  if (Array.isArray(IDList)) {
    ...
    // pass ID string when edit or just need to find index of the only person who has that ID in contactsList
    const IDStr = IDList;
...
const ____editContact = (editedContact, idx) => {
  ...
  contactsList = [...contactsList.slice(0, idx), { ...editedContact }, ...contactsList.slice(idx + 1)];

  isModified = true;
  needToBeReSorted = true;
};
const ____rmContact = (idx) => {
