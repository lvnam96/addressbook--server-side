...
class SignUpFormContainer extends React.Component {
  ...
  onSubmit (e) {
    ...
    const sha512Hash = new JS_SHA('SHA-512', 'TEXT');
    const curState = this.state;

    if (
      curState.passwdVal &&
      curState.cfPasswdVal &&
      curState.passwdVal === curState.cfPasswdVal &&
      curState.isPasswdMatched &&
      !curState.isUnameUsed
    ) {
  ...
  render () {
    ...
    const isReadyToSubmit =
      !!unameVal && !!passwdVal && !!cfPasswdVal && isPasswdMatched === true && isUnameUsed === false;
